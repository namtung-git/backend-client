...
class MultiMessageMqttObserver(wirepas_backend_client.api.MQTTObserver):
    ...
    def generate_gw_status_cb(self)->callable:
        ...
        def on_status_received(message, topic: list):
            ...
            """
            if self.start_signal.is_set():
                message = self.mqtt_topics.constructor("event",
                                                       "status").from_payload(message)
                ...
                if message.state == GatewayState.ONLINE:
                    ...
                    gw_status_msg = {'gw_id':   message.gw_id,
                                     'started': False
                                     }
        ...
        def on_response_cb(message, topic: list):
            ...
            if self.start_signal.is_set():
                ...
                for sink in message.configs:
                    ...
                    if ('sink_id' in sink and
                        'started' in sink and
                        'app_config_seq' in sink and
                        'app_config_diag' in sink and
                        'app_config_data' in sink):
                        gw_status_msg = {'gw_id':   message.gw_id,
...
if __name__ == '__main__':
    ...
    log = LoggerHelper(module_name=__test_name__,
    ...
    daemon.build(storage_name,
                 MySqlStorage,
                 dict(mysql_settings=parser.settings(settings_class=MySQLSettings,
                                                     skip_undefined=False,
                                                    )))
    daemon.set_run(storage_name,
                   task_kwargs={'parallel': True},
                   task_as_daemon=False)

    daemon.build(mqtt_name,
                 MultiMessageMqttObserver,
                 dict(gw_status_queue=gw_status_from_mqtt_broker,
                      mqtt_settings=parser.settings(
                          settings_class=MQTTSettings,
                          skip_undefined=False)),
                 storage=True,
                 storage_name=storage_name)

    daemon.build(control_name,
                 HttpControl,
                 dict(gw_status_queue=gw_status_from_mqtt_broker,
